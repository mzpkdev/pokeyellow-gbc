...
How we mark requirement levels in every markdown an LLM reads - prompts,
skills, agents, hooks. Based on BCP 14 (RFC 2119 + RFC 8174). Keywords are
**levels, not volume** - for emphasis rules see
`PROMPTING_BEST_PRACTICES.md` (a companion guide not currently included in
this repository).
...
Five levels. The RFC's synonyms are legal but we skip them - one word per
level keeps the signal clean.
...
Skipped synonyms: REQUIRED, SHALL (= MUST) / SHALL NOT (= MUST NOT) /
RECOMMENDED (= SHOULD) / NOT RECOMMENDED (= SHOULD NOT) / OPTIONAL (= MAY).

House additions: REQUIRED / OPTIONAL keep one narrow job - labeling inputs,
fields, and arguments, never sentences. ALWAYS / NEVER (not RFC words) mark
every-case, every-turn rules.

---

## Case carries the force

RFC 8174: keywords are normative **only in ALL CAPS**. Lowercase "must" /
"should" is plain English - use it freely when you don't mean a requirement.
...
Litmus-test the violation, not the wish. If you can name a legitimate
exception, it is not a MUST.
...
Name the escape hatch on a SHOULD when you know it: "Responses SHOULD fit one
screen unless the user asked for a full listing."

---

## Dose it - keywords are levels, not volume

RFC 2119 section 6: imperatives are for interoperation and harm-limiting, not
style preferences. Same failure mode as shouting (see the external companion
guide `PROMPTING_BEST_PRACTICES.md`): a doc where everything is MUST teaches
the reader to ignore MUST.

- Plain imperative stays the default voice ("Run `bun test`."). Reach for a
  keyword only when the requirement **level itself** is information.
...
In-repo docs carry no boilerplate - this file is the global declaration. A doc
that leaves the repo (published skill, standalone prompt) SHOULD carry the BCP
14 line:
...
- **Trained prior** - models have read decades of RFC-styled specs; ALL-CAPS
  keywords bind as requirements, not decoration.
- **Compression** - "MUST" replaces "it is critically important that you
  always..." at a fraction of the tokens.
- **Conflict resolution** - graded levels give a tiebreak when instructions
  collide: MUST > SHOULD > MAY; the lower level yields.
